...
function solve(input: string, {size, bytes}: {size: number; bytes: number}) {
  ...
  for (let i = bytes; i < coordinates.length; i++) {
    const grid = createGrid(input, size, i)
    const start = {x: 0, y: 0}
...
run({
  solve,
  tests: [
    {input: 'test.txt', expected: '6,1', size: 7, bytes: 12},
    {input: 'input.txt', expected: '20,12', size: 71, bytes: 1024},
  ],
})
