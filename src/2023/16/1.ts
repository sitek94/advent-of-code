...
function solve(input: string) {
  ...
  const getVisitedKey = (pKey: string, direction: Direction) =>
    `${pKey}:${direction}`
  ...
  const queue = [[start, RIGHT] satisfies [Point, Direction]]
  ...
  while (queue.length) {
    ...
    const visitedKey = getVisitedKey(key, direction)

    const isVisited = visited.get(visitedKey)
    ...
    // Mark as energized
    energized.set(key, (energized.get(key) ?? 0) + 1)
